...
class CountPairs
{
     ...
     */
    static int CountAllPairs(int low, int high)
    {
     ...
     */
    static int CountPairsWithSum(int low, int high, int target)
    {
        int count = 0;
    ...
    /*
     * Enumerate and return all pairs in [low, high] as a list.
     *
     ...
     */
    static List<(int, int)> EnumeratePairs(int low, int high)
    {
        ...
        for (int i = low; i <= high; i++)           // pick the first element
        {
            ...
            {
                pairs.Add((i, j));                  // add the tuple pair to the result
            }
        }
        return pairs;
    }
    ...
    static void Main()
    {
        ...
        Console.WriteLine($"Total pairs: {CountAllPairs(low, high)}\n");

        // --- Demo 2: Pairs that sum to a specific target ---
        int target = 6;
        Console.WriteLine($"Pairs with sum {target}: {CountPairsWithSum(low, high, target)}\n");

        // --- Demo 3: Full enumeration of all pairs ---
        var pairs = EnumeratePairs(low, high);
        Console.WriteLine("All pairs:");
        foreach (var (a, b) in pairs)  // tuple deconstruction
        {
